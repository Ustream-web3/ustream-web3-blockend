...
!developmentChains.includes(network.name)
    ...
    : describe("Vendor Unit Test", () => {
          ...
          beforeEach(async () => {
              ...
              vendorTokensSupply = await tokenContract.balanceOf(
                  vendorContract.address
              )
          ...
          describe("Test buyTokens() method", () => {
              ...
              it("buyTokens success!", async () => {
                  ...
                  const userTokenAmount = ethers.utils.parseEther("10")
          ...
          describe("Test withdraw() method", () => {
              it("withdraw reverted because called by not the owner", async () => {
                  await expect(
                      vendorContract.connect(addr1).withdraw()
                  ).to.be.revertedWith("Ownable: caller is not the owner")
              })

              it("withdraw reverted because called by not the owner", async () => {
                  await expect(
                      vendorContract.connect(owner).withdraw()
                  ).to.be.revertedWith("Owner has not balance to withdraw")
              })
              ...
              it("withdraw success", async () => {
                  ...
                  // withdraw operation
                  const txWithdraw = await vendorContract
                      .connect(owner)
                      .withdraw()
          ...
          describe("Test sellTokens() method", () => {
              ...
              it("sellTokens reverted because vendor has not enough tokens", async () => {
                  ...
                  const amountToSell = ethers.utils.parseEther("10")
                  await expect(
                      vendorContract.connect(addr1).sellTokens(amountToSell)
                  ).to.be.revertedWith(
